...
def set_ip_range():
    while True:
        ...
        try:
            ...
            time.sleep(2)
#--------------------------- -------------------------------------------------------------------------
def save_nmap_results(ip, output):
    
    data = read_json("nmap_results.json") or {}
    data[ip] = output  # ذخیره خروجی اسکن برای هر IP

    with open("nmap_results.json", "w") as f:
        json.dump(data, f, indent=4)

    print(Fore.GREEN + f"Nmap results saved for {ip} in nmap_results.json")

def show_nmap_results():
    data = read_json("nmap_results.json")
    if not data:
        print(Fore.RED + "No previous Nmap results found!")
        return
...
def auto_scan():
    ...
    print(Fore.GREEN + f"Auto-Scanning network: {ip_range} every 10 minutes...")

    while True:
        network_scan()
        print(Fore.YELLOW + "Waiting 10 minutes before next scan...")
        time.sleep(600)  # 600 ثانیه = 10 دقیقه
...
def services_menu():
    ...
    while True:
        print(Fore.CYAN + "\n[ 1 ] OS Detection (-O)")
        print(Fore.CYAN + "[ 2 ] Aggressive Scan (-A)")
        print(Fore.CYAN + "[ 3 ] Service Detection (-sV)")
        print(Fore.CYAN + "[ 4 ] Scan Specific Ports (-p)")
        print(Fore.CYAN + "[ 5 ] Nmap Scripts (NSE)")
        print(Fore.CYAN + "---------------------")
        ...
        if choice in ["1", "2", "3", "4", "5"]:
            for device in scan_data:
                ...
                if choice == "1":
                    ...
                    command = ["sudo", "nmap", "-p", ports, ip]
                elif choice == "5":
                    script = input(Fore.YELLOW + "Enter NSE Script Name: ")
                    command = ["sudo", "nmap", "--script", script, ip]
                ...
                save_nmap_results(ip, result.stdout)
...
def main_menu():
    while True:
        ...
        print(Fore.CYAN + "[ 3 ] Services (Nmap & Ping)")  
        print(Fore.CYAN + "[ 4 ] Show Scan Results")  
        print(Fore.CYAN + "[ 5 ] Show Targets")  
        print(Fore.CYAN + "[ 6 ] Show Previous Nmap Results")  # جدید
        print(Fore.CYAN + "[ 7 ] Start Auto-Scan (10 min interval)")  # جدید
        print(Fore.CYAN + "[ 00 ] Exit")  
        print("\n----------------------------------------------------------------------------")

        choice = get_valid_input("Your Choice? : ", ["1", "2", "3", "4", "5", "6", "7", "00"])
        ...
        if choice == "1":
            ...
            services_menu()  # الان اینجا دیده میشه ✅
        elif choice == "4":
            show_scan_results()  # نتایج اسکن نمایش داده میشن ✅
        elif choice == "5":
            show_targets()  # لیست اهداف نمایش داده میشه ✅
        elif choice == "6":
            show_nmap_results()  # نتایج قبلی Nmap نمایش داده میشن ✅
        elif choice == "7":
            auto_scan()  # اسکن خودکار اجرا میشه ✅
        elif choice == "00":
            print(Fore.RED + "\nExiting... Goodbye!")
            time.sleep(1)
            clear_screen()
